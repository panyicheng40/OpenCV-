...
vc = cv2.VideoCapture("C:\\Users\\panyi\\Desktop\\Work\\bd_movie\\data\\EXmovie.mp4")  # 读入视频文件
# vc = cv2.VideoCapture("C:/Users/jason/Desktop/152821AA.MP4")

src, firstFrame = vc.read()
...
while True:
    ...
    cv2.imshow("prve_frame", prve_frame)

    # 计算当前帧与上一帧的差别
    frameDiff = cv2.absdiff(prve_frame, gray_frame)
    cv2.imshow("frameDiff", frameDiff)
    prve_frame = gray_frame.copy()
    ...
    cv2.putText(frame, "F{}".format(30), (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
